...
vector<vector<int>> NFAToTable(NFA* nfa)
{
    ...
    for(vector<graph::State *>::iterator iter = graph::allStates.begin(); iter != graph::allStates.end(); ++iter)
    {
        ...
        for(vector<pair<graph::State*, string>>::iterator iter2 = nextStates.begin(); iter2 != nextStates.end(); ++iter2)
        {
            dest = iter2->first;
            trans = iter2->second;
            ...
            vector<int> row = TransitionTableConverter::transTable.at(srcInt);
            it = row.begin();
